namespace Endjin.CucumberJs.TestAdapter
{
    ...
    public sealed class TestFileAddRemoveListener : IVsTrackProjectDocumentsEvents2, IDisposable, ITestFileAddRemoveListener
    {
        ...
        public void StartListeningForTestFileChanges()
        {
            ...
            {
                int hr = this.projectDocTracker.AdviseTrackProjectDocumentsEvents(this, out this.cookie);
        ...
        {
            if (this.cookie != VSConstants.VSCOOKIE_NIL && this.projectDocTracker != null)
            {
                int hr = this.projectDocTracker.UnadviseTrackProjectDocumentsEvents(this.cookie);
        ...
        {
            return this.OnNotifyTestFileAddRemove(cProjects, rgpProjects, rgpszMkDocuments, rgFirstIndices, TestFileChangedReason.Added);
        }
            ...
            TestFileChangedReason reason)
        {
            ...
            for (int changeProjIndex = 0; changeProjIndex < changedProjectCount; changeProjIndex++)
            {
                ...
                {
                    if (changedProjects[changeProjIndex] != null && this.TestFileChanged != null)
                    {
